...
) -> crate::Result<super::SessionInfo> {
    let refresh_config = refresh_config.unwrap_or_default();
    let mut last_error = None;
    let mut rng = SmallRng::from_entropy();

    for attempt in 0..refresh_config.max_retries {
        match super::refresh_session(config, refresh_token).await {
            Ok(session) => return Ok(session.session),
            Err(e) => {
                ...
                if attempt < refresh_config.max_retries - 1 {
                    ...
                    let base_delay = refresh_config.initial_delay_ms.saturating_mul(2_u64.pow(attempt));
                    let max_delay = refresh_config.max_delay_ms;
                    let actual_delay = base_delay.min(max_delay);
                    
                    // Add jitter (±10%)
                    let jitter = (rng.gen::<f64>() * 0.2 - 0.1) * actual_delay as f64;
