...
use crate::user_roles::{get_permissions_for_role, get_roles_for_user};
...
#[async_trait]
impl<S> FromRequestParts<S> for OptionalSuperTokensUser
where
    S: Send + Sync,
{
    type Rejection = ();
...
#[async_trait]
impl<S> FromRequestParts<S> for RequiredSuperTokensUser
where
    S: Send + Sync,
{
...
#[async_trait]
impl<S> FromRequestParts<S> for SuperTokensSessionExtractor
where
    S: Send + Sync,
{
    type Rejection = SuperTokensError;
...
pub fn require_roles_middleware(
    roles: Vec<String>,
) -> impl Fn(Extension<OptionalSuperTokensUser>, Request, Next) -> Result<Response> + Clone {
...
pub fn supertokens_layer(
    ...
    impl Fn(
        Cookies,
        Extension<SuperTokensMiddlewareConfig>,
        Request,
        Next,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Response>> + Send>>,
    (Cookies, Extension<SuperTokensMiddlewareConfig>),
> {
    axum::middleware::from_fn_with_state(
        ...
        |Extension(config): Extension<SuperTokensMiddlewareConfig>,
         ...
         next: Next| {
            Box::pin(
                async move { supertokens_middleware(cookies, Extension(config), req, next).await },
            )
        },
    )
}
...
mod tests {
    ...
    fn create_test_config() -> SuperTokensConfig {
        SuperTokensConfig {
            ...
            supertokens_uri: "http://localhost:3567".to_string(),
            api_key: None,
